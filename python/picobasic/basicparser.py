...
class BASICParser:
    ...
    def __simplestmt(self):
        ...
        elif self.__token.category == Token.GETCHR:
            self.__getchrstmt()
            return None
        ...
        elif self.__token.category == Token.CLR:
            Term.clear()
            Term.home()
            return None
        ...
        elif self.__token.category == Token.TONE:
            self.__tonestmt()
            return None
    ...
    def __cursorstmt(self):
        ...
        if self.__tokenindex + 2 > len(self.__tokenlist):
            raise RuntimeError(
                "Expecting X and Y positions on line " + str(self.__line_number)
            )
        else:
            # get the X
            self.__logexpr()
            ...
            Term.cursor(xpos, ypos)

    def __tonestmt(self):
        """Plays a tone of a specific freq
        and duration
        """
        self.__advance()
        if self.__tokenindex + 2 > len(self.__tokenlist):
            raise RuntimeError(
                "Expecting tone and duration in line " + str(self.__line_number)
            )
        else:
            # get the tone
            self.__logexpr()
            tone = self.__operand_stack.pop()

            # get the duration
            self.__logexpr()
            duration = self.__operand_stack.pop()

            Buzzer.play(tone, duration)
    ...
    def __printstmt(self):
        ...
        if not self.__tokenindex >= len(self.__tokenlist):
            self.__logexpr()
            Term.write(str(self.__operand_stack.pop()))
    ...
    def __getchrstmt(self):
        """Gets a single char as a integer"""
        self.__advance()  # Advance past INPUT token

        # Acquire the variable to store
        if not self.__tokenindex >= len(self.__tokenlist):
            if self.__token.category != Token.NAME:
                raise ValueError(
        ...
        tmpval = Keyboard.get_char()
        if type(tmpval) != int:
            tmpval = ord(tmpval)
        self.__symbol_table[name] = tmpval
    ...
    def __inputstmt(self):
        ...
        while not valid_input:
            ...
            Term.write(prompt)
            inputstring = Keyboard.get_line()
            inputvals = inputstring.split(",", (len(variables) - 1))
            ...
            for variable in variables:
                ...
                try:
                    ...
                    valid_input = False
                    Term.print("Not enough values input - redo from start")
                    break
    ...
    def __readstmt(self):
        ...
        if len(variables) > len(self.__data_values):
            raise RuntimeError(
                "Insufficient constants supplied to READ "
                + "in line "
                + str(self.__line_number)
            )
    ...
    def __evaluate_function(self, category):
        ...
        if category == Token.RND:
            ...
            self.__consume(Token.RIGHTPAREN)
            # Vintage basic reseeds with negative values
